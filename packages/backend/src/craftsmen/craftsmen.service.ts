...
import { PatchRequest, Response } from '@not-so-software/shared';
...
export class CraftsmenService {
    constructor(
        ...
        @InjectRepository(CraftsmenPostal)
        private readonly craftsmenPostalRepository: Repository<CraftsmenPostal>,
    ) {}
    ...
    async update(id: number, payload: PatchRequest) {
        ...
        try {
            ...
            if (!craftsman) {
                throw new NotFoundException(
                    `Craftsman with ID ${id} not found`,
                );
            }
            ...
            oldRankings.forEach(async (entry) => {
                const distanceScore =
                    1 - (entry.distance * 1.0) / defaultDistance;
                const distanceWeight =
                    entry.distance > defaultDistance ? 0.01 : 0.15;
